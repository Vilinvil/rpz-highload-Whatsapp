...
| Запросы на регистрацию                   |    3    |     4.5     |             9              |     0.04     |            0.08             |
| Запросы на проверку сессии               | 78 081  |   117 122   |          234 243           |    0.028     |            0.056            |
| Запросы на в службу поддержки            |   12    |     18      |             36             | не учитываем |        не учитываем         |
...
| Файловое хранилище за год                                |   388.4 Пбайт    |             466.1 Пбайт              |
...
13 258 Тбайт в день * 30 д / 1024 = 388.4 Пбайт
...
| Сервис                                          |        Целевой RPS        |  CPU   |   RAM    |     Net      | 
|-------------------------------------------------|:-------------------------:|:------:|:--------:|:------------:|
| web && mobile nginx                             | RPS 869 096 и 234 243 CPS |  548   |   5 Гб   | 8410 Гбит/с  |
| web && mobile api gateway                       |         1 103 339         |  482   |    -     | 8410 Гбит/с  |
| auth.auth                                       |          301 257          |  276   |   4 Гб   |   8 Гбит/с   |
| auth.limitation_worker                          |            36             |   1    |  15 Мб   | 0.28 Мбит/с  |
| user.user                                       |             9             |   1    |  15 Мб   |  0.1 Мбит/с  |
| chat.chat                                       |          935 068          | 11 689 | 1 712 Гб |  4  Гбит/с   |
| chat.attachments                                |          84 374           | 1 055  | 1 545 Гб | 8380 Гбит/с  |
| chat.removing_message                           |          561 457          | 7 019  | 1 029 Гб |   4 Гбит/с   |
| chat.message, chat.view, chat.status  workers   |          802 082          |  201   |   3 Гб   |   4 Гбит/с   |
| chat.reaction worker                            |          67 014           |   17   |  0.3 Гб  | не учитываем |
| call.call, stun&&turn                           |            692            |   9    |  1.4 Гб  |  26 Гбит/с   |
| support.support                                 |            36             |   1    |  0.2 Гб  |  0.3 Мбит/с  |
...
Суммарный трафик = 4 + 2 670 + 232 + 5 424 + 54 + 26 + 0.08 = 8410 Гбит/с
...
Предположим envoy api gateway в 5 раз менее производителный чем nginx, т.к. имеет более сложную логику
Тогда нам нужно 869 096 + 234 243 = 1 103 339 RPS * 5 / 11 455 = 482 ядра.
...
auth.auth: считаем легким JSON API. Идет весь трафик, за исключением вебсокета.
CPU = (1 103 339 - 802 082) / 4000 = 76 ядер;
RAM = 76 * 15 Мб = 1.1 Гб;
Net = 1 Кбайт * (1 103 339 - 802 082) = 2.3 Гбит/с.

auth.limitation_worker: считаем легким JSON API
CPU = 36 / 4000 = 1;
RAM = 1 * 15 МБ = 15 Мб;
Net = 1 Кбайт * 36 = 0.28 Мбит/с.

user.user: считаем легким JSON API
CPU = 9 / 4000 = 1;
RAM = 1 * 15 МБ = 15 Мб;
Net = 1 Кбайт * 9 = 0.1 Мбит/с.

chat.chat: считаем средней бизнес логикой
CPU = (802 082 + 48 612 + 67 014 + 11 112 + 5 208 + 1 040) / 80 = 11 689 ядер;
RAM = 11 689 * 150 МБ = 1 712 Тб;
Net = 4 Гбит/с.

chat.attachments: считаем тяжелой бизнес логикой
CPU = (67 014 + 11 112 + 5 208 + 1 040) / 8 = 10 547 ядер;
RAM = 10 547 * 150 МБ = 1 545 Гб;
Net = (2 670 + 232 + 5 424 + 54) = 8380 Гбит/с.
...
**Amazon S3**:
Т.к. оседает по предположению 30% трафика файлов:
(1335 + 116 + 2712 + 27)Гбит/с * 0.3 * (24д * 60мин * 60с) / (8 * 1024) = 13 248 Тбайт в день

**Cassandra**:
Прирост из-за сообщений
2 Гбит/с * (24д * 60мин * 60с) / (8 * 1024) = 21.1 Тбайт в день 
