...
2. Детектирование с помощью алгоритмов машинного обучения запрещенных видов (призывы к терроризму и т.д.) 
...
3. Сжатие (только для видео). Видео будет использоваться с форматом mp4 с кодеком H.264 ввиду его
...
| Signal Protocol      | Backend & Client |                    Протокол для шифрования был реализован Singal, Facebook мессенджерами и WhatApp. Плюсом является защита от компроментирования предыдущих и следующих сообщений при утечки текущего ключа                                             |
| SQLite               |      Client      |                                            Используем локальное персистентное хранилище на клиенте, т.к. реализуем отсутствие хранения сообщений на сервере.                                             |
| React                |     Frontend     |                                                                             Фреймворк от Meta** является широкоиспользуемым.                                                                             |
