...
а сервис removing attachment будет периодически запускать Cron джобы, которые будут ходить в основное хранилище, занулять
там поля, а потом ходить удалять вложения из Amazon S3. 

Db reader - микросервисы для разграничения доступа на чтение в Cassandra. 

Auxiliary - микросервисы для работы с редко меняющимися таблицами: user, chat, folder, report_abuse.
