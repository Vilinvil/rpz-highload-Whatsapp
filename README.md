...
и Turn сервера, который требует проксировать через себя запросы, но позволяет обойти симметричный Nat.
...
необходимости доступ к Cassandra. Attachments - микросервисы будут использоваться реализиции пайплайнов загрузки, 
получения и удаления вложений, используя Amazon S3. Здесь будет использоваться Websocket протокол
для чата.

Через **Kafka** будет обрабатываться запросы на удаления файлов вложений, события просмотров сообщений, 
события жалоб от пользователей.
